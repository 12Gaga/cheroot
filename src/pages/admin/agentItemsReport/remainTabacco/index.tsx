...
const RemainTabacco = () => {
  ...
  const cheroots = useAppSelector((store) => store.typeOfCheroot.item)
    ...
    .sort((a, b) => a.id - b.id);
  const remainTabaccos = useAppSelector(
  ...
  const [remainTabacco, setRemianTabacco] = useState<AgentLeftTabacco[]>([]);
  const [compensationTabacco, setCompensationTabacco] = useState<
    CompensationTabacco[]
  >([]);
  const handelDate = (date: Date) => {
    ...
    const concernRemainTabacco = remainTabaccos.filter(
    ...
    setRemianTabacco(concernRemainTabacco);
    setCompensationTabacco(concernCompensation);
  };
      ...
      <Box sx={{ width: "90%", margin: "0 auto" }}>
        <table border={1} className="table">
          ...
          <tr>
            ...
            <th style={{ backgroundColor: "#FFDA78" }}>လက်ကျန်ဆေးစပ်(ပြည်)</th>
            {cheroots.map((item) => {
            ...
            <td style={{ textAlign: "center", backgroundColor: "#FFDA78" }}>
              {remainTabacco.reduce((tol, f) => {
                return (tol += f.tin * 16 + f.pyi);
              }, 0)}
            </td>
          </tr>
        </table>
      ...
      <Box sx={{ mt: 10 }}>
        ...
        <table border={1}>
          <tr>
            ...
            <th style={{ width: 150, backgroundColor: "#95D2B3" }}>ရော်ပြည်</th>
          </tr>
          {compensationTabacco.map((item) => {
            return (
              <tr key={item.id}>
                <th>{(no += 1)}</th>
                <td style={{ textAlign: "center" }}>
          ...
          <tr>
            <td colSpan={3}></td>
            <td style={{ textAlign: "center", backgroundColor: "#FFDA78" }}>
              {compensationTabacco.reduce((tol, co) => {
                return (tol += co.compensationPyi);
              }, 0)}
            </td>
            <td style={{ textAlign: "center", backgroundColor: "#FFDA78" }}>
              {compensationTabacco.reduce((tol, co) => {
                return (tol += co.takeMoneyPyi);
              }, 0)}
            </td>
          </tr>
        </table>
