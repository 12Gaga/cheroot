...
const RemainLabel = () => {
  ...
  const cheroots = useAppSelector((store) => store.typeOfCheroot.item)
    ...
    .sort((a, b) => a.id - b.id);
  const remainLabels = useAppSelector(
  ...
  const [remainLabel, setRemianLabel] = useState<AgentLeftLabel[]>([]);
  const [compensationLabel, setCompensationLabel] = useState<
    CompensationLabel[]
  >([]);
  const handelDate = (date: Date) => {
    ...
    const concernRemainLabel = remainLabels.filter(
    ...
    setRemianLabel(concernRemainLabel);
    setCompensationLabel(concernCompensation);
  };
      ...
      <Box sx={{ width: "90%", margin: "0 auto" }}>
        <table border={1} className="table">
          ...
          <tr>
            ...
            <th style={{ backgroundColor: "#FFDA78" }}>လက်ကျန်တံဆိပ်</th>
            {cheroots.map((item) => {
      ...
      <Box sx={{ mt: 10 }}>
        ...
        <table border={1}>
          <tr>
            ...
            <th style={{ width: 150, backgroundColor: "#95D2B3" }}>ရော်လိပ်</th>
          </tr>
          {compensationLabel.map((item) => {
            return (
              <tr key={item.id}>
                <th>{(no += 1)}</th>
                <td style={{ textAlign: "center" }}>
          ...
          <tr>
            <td colSpan={3}></td>
            <td style={{ textAlign: "center", backgroundColor: "#FFDA78" }}>
              {compensationLabel.reduce((tol, co) => {
                return (tol += co.compensationBandle);
              }, 0)}
            </td>
            <td style={{ textAlign: "center", backgroundColor: "#FFDA78" }}>
              {compensationLabel.reduce((tol, co) => {
                return (tol += co.takeMoneyBandle);
              }, 0)}
            </td>
          </tr>
        </table>
