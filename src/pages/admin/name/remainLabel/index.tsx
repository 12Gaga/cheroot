...
const AgentRemainLabelPage = () => {
  ...
  const concernLeftLabel = leftLabel
  ...
  const [concernData, setConcernData] = useState<AgentLeftLabel[]>([]);
  let no = 0;
  const handleAgent = (agentId: number) => {
    const data = concernLeftLabel
  ...
  useEffect(() => {
    setConcernData(concernLeftLabel);
  }, [leftLabel]);
  if (!session) return;
  return (
    <>
        ...
        <Box sx={{}}>
          <table border={1}>
            <tr>
              ...
              <th style={{ width: 150 }}>လက်ကျန်လိပ်</th>
            </tr>
